...
LOCAL = "/mnt/osmanthus/aklharas/checkpoints/mt/checkpoint-37500"
...
def preprocess_function(examples, tokenizer):
    ...
    return {k: v.squeeze(0) for k, v in models_inputs.items()}
...
def run(train=False, test=False, eval=False):
    ...
    validation_ds = validation_ds.select(range(300))
    ...
    if train:
        ...
        with torch.no_grad():
            ...
            labels = [[l] for l in test_ds["ja_sentence"]]
            test_ds = test_ds["en_sentence"]
            tokenized = tokenizer(test_ds, return_tensors="pt", padding=True, truncation=True, max_length=128).to(device)
            ...
            nltk_bleu_score = corpus_bleu(labels, res)
            print({"bleu": nltk_bleu_score})
